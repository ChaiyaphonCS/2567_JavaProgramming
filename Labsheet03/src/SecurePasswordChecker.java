...
public class SecurePasswordChecker {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (!hasUpperCase) {
				errors +="- Password must be at least one uppercase leter (A-Z).\n";
			}
			if (!hasLowerCase) {
				errors +="- Password must be at least one lowercase letter (a-z).\n";
			}
			if (!hasDigit) {
				errors +="- Password must be at least one ditgit (0-9).\n";
			}
